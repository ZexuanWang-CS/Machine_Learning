    ...
    for itra in range(args.iterations):
        for dp in range(len(train_ys)):
            ...
            for outNeuron in range(len(model[-1][2])):
                ...
                model[-1][3][outNeuron] = (model[-1][1][outNeuron]-train_ys[dp])*model[-1][1][outNeuron]*(1.0-model[-1][1][outNeuron])
            for layerBP in range(len(model)):
                for neuronBP in range(len(model[layerBP][0])):
                    if layerBP == len(model)-1:
                        ...
                        model[layerBP][0][neuronBP] = model[layerBP][0][neuronBP] - args.lr*inputBP*model[layerBP][3][neuronBP]
                    else:
                        weightBPls = []
                        for weightBP in model[-1][0]:
                            weightBPls.append(weightBP[neuronBP])
                        update = np.sum(model[-1][3].dot(weightBPls))*model[layerBP][1][neuronBP]*(1.0-model[layerBP][1][neuronBP])*train_xs[dp, :, :]
